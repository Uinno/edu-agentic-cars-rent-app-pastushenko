...
- **UI library:** shadcn/ui (Radix UI primitives + Tailwind CSS) — provides `Button`, `Card`, `Badge`, `Input`, `Label`, `Dialog`, `Table`, `Alert`, `ToggleGroup`, `Separator`
...
│   ├── ui/         # shadcn/ui auto-generated primitives (Button, Card, Badge, …)
│   ├── auth/
│   ├── cars/
│   ├── rentals/
│   └── layout/
...
├── lib/            # shadcn/ui utils (cn helper generated by init)
...
**Deliverable:** All required packages installed, Tailwind CSS configured, and shadcn/ui initialized.
...
**Step 3 — Add Tailwind CSS (via Vite plugin):**
...
Add Tailwind import to `frontend/src/index.css` (replace contents):
...
When prompted, select: **New York** style, **Neutral** base color, CSS variables **yes**. This creates `src/components/ui/`, `src/lib/utils.ts` (the `cn()` helper), and `components.json`.
...
**Expected result:** `src/components/ui/` contains at least 10 primitive component files; `package.json` lists `class-variance-authority`, `clsx`, `tailwind-merge`, and `@radix-ui/*` packages.
...
- `LoginPage` / `RegisterPage`: wrap form in shadcn `<Card>` + `<CardHeader>` / `<CardContent>`; use `<Input>`, `<Label>`, `<Button>` from `components/ui`; display auth errors with shadcn `<Alert variant="destructive">`
- `ProtectedRoute` loading state: render a centered spinning `<div>` or a shadcn `<Separator>` — no third-party spinner needed
...
- Wrap in shadcn `<Card>` + `<CardHeader>` / `<CardContent>` / `<CardFooter>`
- Display: brand + model + year, price/day formatted with `formatCurrency()`
- Availability: shadcn `<Badge variant="default">` (green) for available, `<Badge variant="secondary">` for unavailable
- If `distance_meters` present: show `formatDistance()` result as muted text beside the badge
- "Rent" button: shadcn `<Button>` (only if `isAvailable && onRent` provided)

`frontend/src/components/cars/RadiusFilter.tsx`

- Props: `value: 5|10|15`, `onChange: (r: 5|10|15) => void`, `disabled?: boolean`
- Use shadcn `<ToggleGroup type="single">` with three `<ToggleGroupItem>` values for 5km, 10km, 15km
...
- **Rent modal**: clicking "Rent" on a `CarCard` opens a shadcn `<Dialog>` with `<DialogContent>` containing a form with shadcn `<Input type="date">` fields for startDate + endDate and a `<Button type="submit">`
  - Call `createRental({ carId, startDate, endDate })`
  - On success: close dialog, show shadcn `<Alert>` success banner, refresh car list
  - On error (409 conflict): show shadcn `<Alert variant="destructive">` inside the dialog: "Car already booked for selected dates"
  - `console.debug('[CarsPage] Rental submitted:', { carId, startDate, endDate })`
- **Error state**: if geolocation denied, show shadcn `<Alert variant="destructive">` "Enable location access to search nearby cars"
...
- Wrap in shadcn `<Card>` + `<CardContent>`
- Display: car brand + model, dates formatted with `formatDate()`, shadcn `<Badge>` for status mapped to variants: `pending→outline`, `active→default`, `completed→secondary`, `cancelled→destructive`
- "Cancel" button: shadcn `<Button variant="destructive" size="sm">` for `pending` rentals (if `onCancel` provided)
...
- Table view: use shadcn `<Table>` + `<TableHeader>` / `<TableBody>` / `<TableRow>` / `<TableCell>` — columns: brand, model, year, price, available (`<Badge>`), location
- **Add car** form: shadcn `<Dialog>` triggered by a top-right `<Button>Add Car</Button>`; form fields use `<Input>` + `<Label>`; `createCar(data)` on submit, refresh list
- **Delete** button per row: shadcn `<Button variant="destructive" size="icon">` opens a confirm `<Dialog>` before calling `deleteCar(id)`
  - `console.debug('[AdminCarsPage] Deleting car:', id)`
- **Toggle availability**: inline shadcn `<Button variant="outline" size="sm">` PATCH car with `isAvailable` flipped
...
- Table: use shadcn `<Table>` — columns: renter email, car (brand + model), start/end dates, shadcn `<Badge>` status
- "Complete" button: shadcn `<Button size="sm">` per row; `completeRental(id)`, refresh
  - `console.debug('[AdminRentalsPage] Completing rental:', id)`
- Tab or toggle for "All Rentals" (calls `getAllRentals()`) — use two shadcn `<Button variant="outline/default">` pills as a toggle

`frontend/src/pages/AdminUsersPage.tsx`

- On load: `getUsers()`
  - `console.debug('[AdminUsersPage] Fetching users')`
- Table: use shadcn `<Table>` — columns: email, firstName, lastName, `<Badge>` role, created date
- "Remove" button: shadcn `<Button variant="destructive" size="sm">` per row with confirm `<Dialog>`; calls `deleteUser(id)` (soft delete on backend)
  - `console.debug('[AdminUsersPage] Removing user:', id)`
- Filter/search: shadcn `<Input placeholder="Search by email…">` for client-side filtering
